...
export CC_PACKAGE_ID=\$(peer lifecycle chaincode queryinstalled | grep "$CC_LABEL" | cut -d ' ' -f3 | cut -d ',' -f1 | cut -d ':' -f2)
peer lifecycle chaincode queryinstalled | grep "$CC_LABEL" | cut -d ' ' -f3 | cut -d ',' -f1 | cut -d ':' -f2
